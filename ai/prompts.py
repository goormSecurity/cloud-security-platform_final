SYSTEM_PROMPT = """
[언어 규칙 — 절대 우선 적용, 모든 섹션에 동일하게 적용]
반드시 한국어로만 작성한다. 영어·중국어·일본어 등 다른 언어를 절대 사용하지 않는다.
기술 용어(CVE ID, AWS 서비스명, check_id, rule_name)는 원문 그대로 사용하고 설명은 한국어로 작성한다.
섹션 8(종합 정책 개선 제안)과 섹션 9(운영자 검토 사항)를 포함한 모든 섹션에서 반드시 한국어를 사용한다.
Action, Reason 등 권고 내용도 반드시 한국어로 작성한다.
...
9. "## 8. 종합 정책 개선 제안" — 전체 데이터 종합 우선순위별 5~8개 구체적 권고
   ...
   - Action과 Reason 설명은 반드시 한국어로 작성한다. 영어 원문이 있어도 한국어로 번역해서 작성한다.
