#!/bin/bash
# ============================================================
# 보안 취약점 수정 스크립트 — 2026-07-07 06:24 KST
...
# ── [10] [CVE-2026-31789] openssl: OpenSSL: Heap buffer overflow on 32-bit systems from large X.509 certif (CRITICAL) ──
# 식별자: CVE-2026-31789
# 리소스: ghost:5-alpine
...
# ── [11] [CVE-2018-1333] httpd: mod_http2: Too much time allocated to workers, possibly leading to DoS (HIGH) ──
# 식별자: CVE-2018-1333
# 리소스: vulnerables/web-dvwa
...
# ── [12] [CVE-2018-17199] httpd: mod_session_cookie does not respect expiry time (HIGH) ──
# 식별자: CVE-2018-17199
# 리소스: vulnerables/web-dvwa
...
# ── [13] [NSWG-ECO-428] Out-of-bounds Read (HIGH) ──
# 식별자: NSWG-ECO-428
# 리소스: bkimminich/juice-shop
...
# ── [14] [CVE-2020-15084] Authorization bypass in express-jwt (HIGH) ──
# 식별자: CVE-2020-15084
# 리소스: bkimminich/juice-shop

# CVE-2020-15084 — Authorization bypass in express-jwt
# Image: bkimminich/juice-shop
# 패키지: express-jwt 0.1.3 → 6.0.0
# 방법 1: 베이스 이미지 업그레이드 후 재빌드
docker pull bkimminich/juice-shop:latest
docker build --no-cache -t bkimminich/juice-shop:patched .

# 방법 2: 컨테이너 내 패키지만 업데이트 (임시 조치)
# apt-get update && apt-get install -y --only-upgrade express-jwt
